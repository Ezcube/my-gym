...
function ExercisePicker({ onPick, close }) {
  ...
  if (bp === '★') base = [...base].sort((a, b) => (usage[b.id] - usage[a.id]) || exerciseName(a).localeCompare(exerciseName(b), getLang()))
...
function PlanTools({ close }) {
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own My Gym — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
  ...
  return <>
    ...
    {w.entries.map((e, i) => {
      ...
      return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
        {ex && <Thumb ex={ex} />}
        <div className="grow"><div className="tt capitalize" style={{ fontWeight: 600 }}>{exerciseName(ex || { id: e.id, n: e.muscleSnapshot?.n || e.n || e.id })} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>}</div>
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {(prs.length > 0 || e1prs.length > 0) && <div style={{ textAlign: 'left', marginBottom: 12 }}>
      {prs.map(id => <div key={id} className="small accent capitalize row" style={{ gap: 5 }}><Icon name="trophy" style={{ fontSize: 13 }} />{t('New PR:')} {exerciseName(EXIDX[id] || { id, n: id })}</div>)}
      {e1prs.map(p => <div key={p.id} className="small accent capitalize row" style={{ gap: 5 }}><Icon name="chartLine" style={{ fontSize: 13 }} />{t('Best estimated 1RM:')} {exerciseName(EXIDX[p.id] || { id: p.id, n: p.id })} · {fmtNum(p.est)} {st.unit}</div>)}
    </div>}
