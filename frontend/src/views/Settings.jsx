...
import HealthPairingCard, { canShowHealthPairing } from '../components/HealthPairingCard.jsx'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error(t('this isn’t a My Gym backup file'))
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host My Gym')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    {canShowHealthPairing({ user, demo: DEMO, mobile: MOBILE }) && <HealthPairingCard confirm={confirmSheet} />}
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install My Gym as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      ...
      exercise visuals: original generated guidance for 165 exercises + built-in MuscleMap fallback (MIT-derived)
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if My Gym is closed.')}>
