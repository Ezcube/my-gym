...
test('every approved id produces both complete prompts', () => {
  assert.equal(EXERCISE_VISUAL_IDS.length, 696)
...
test('cable kayak row, arm-blaster pushdown, and trunk-rotation batch preserves details', () => {
  const technique = {
    '2464': /low pulley.*sit facing.*feet flat.*one hand.*arm fully extended.*slight elbow bend.*hinge forward.*straight back.*braced abs.*retracting the shoulder blade.*elbow close.*switch sides/i,
    '2405': /V-bar.*highest pulley.*arm blaster.*upper arms.*torso.*facing the machine.*feet shoulder-width.*overhand.*palms-down.*full elbow extension.*return/i,
    '0242': /low pulley.*lie face up.*mat.*both hands.*arms straight above the chest.*knees.*chest.*curl the pelvis.*lift the hips.*lower/i,
    '0243': /facing the cable machine.*feet shoulder-width.*both hands.*front of the chest.*elbows slightly bent.*brace the core.*twist.*right.*across the body.*alternate sides/i,
    '0862': /facing the cable machine.*feet shoulder-width.*both hands.*front of the chest.*elbows slightly bent.*hips and legs stable.*rotating the torso.*alternate sides/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '2464': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Rhomboids, Rear deltoids/i,
    '2405': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0242': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Hip flexors/i,
    '0243': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques/i,
    '0862': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques, Lower back/i,
  }
...
test('cable adduction, incline press/fly, stability-ball fly, and pushdown batch preserves details', () => {
  const technique = {
    '0168': /ankle cuff.*facing the cable machine.*leg straight.*across the body.*midline/i,
    '0169': /45-degree incline.*high pulleys.*facing the machine.*shoulder height.*forward and upward/i,
    '0171': /low position.*incline bench.*back supported.*overhand.*slightly bent elbows.*sides/i,
    '0170': /stability ball.*high pulleys.*facing away.*palms forward.*parallel to the ground/i,
    '0172': /straight bar.*high pulley.*facing away.*overhand.*wider than shoulders.*thighs.*extending the elbows/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0168': /Primary muscles: Adductors\.[\s\S]*Secondary muscles: Glutes, Quadriceps/i,
    '0169': /Primary muscles: Pectorals\.[\s\S]*Secondary muscles: Deltoids, Triceps/i,
    '0171': /Primary muscles: Pectorals\.[\s\S]*Secondary muscles: Deltoids, Triceps/i,
    '0170': /Primary muscles: Pectorals\.[\s\S]*Secondary muscles: Deltoids, Triceps/i,
    '0172': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('cable incline extension, judo flip, kickback, kneeling row, and kneeling extension preserve details', () => {
  const technique = {
    '0173': /low pulley.*facing away.*arms overhead.*elbows close.*behind the head.*extend back overhead/i,
    '0174': /facing the cable machine.*both hands.*chest height.*rotate.*pivoting.*over the shoulder/i,
    '0860': /one cable handle.*hinge forward.*flat back.*upper arm close.*forearm backward/i,
    '3697': /kneel facing.*low cable pulley.*rope.*neutral grip.*upper chest.*shoulder blades/i,
    '0176': /rope.*high pulley.*kneel facing.*beside the head.*upper arms fixed.*forearms/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0173': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0174': /Primary muscles: Obliques\.[\s\S]*Secondary muscles: Shoulders/i,
    '0860': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '3697': /Primary muscles: Delts\.[\s\S]*Secondary muscles: Trapezius, Rhomboids, Biceps/i,
    '0176': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('cable pulldown, low fly, and lying curl batch preserves details', () => {
  const technique = {
    '0177': /rope.*high pulley.*facing the machine.*overhand.*palms facing each other.*sides/i,
    '2616': /lat pulldown.*thighs secured.*V-bar.*overhand.*upper chest.*elbows close/i,
    '0179': /handles.*low pulleys.*centered.*overhand.*arms out.*cross in front of the chest/i,
    '1634': /straight bar.*low pulley.*face up.*flat bench.*underhand.*elbows close.*shoulders/i,
    '0182': /straight bar.*low pulley.*face up.*flat bench.*close underhand.*elbows close.*chest/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0177': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '2616': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
...
test('single-arm cable curl, raises, preacher curl, and pulldown preserve details', () => {
  const technique = {
    '0190': /facing the cable machine.*one low-pulley handle.*underhand.*palm-up.*elbow close.*forearm/i,
    '0191': /cable machine.*one handle.*hinge forward.*straight back.*arm out to the side.*parallel/i,
    '0192': /cable machine.*one handle.*palm down.*arm straight.*side.*shoulder height/i,
    '1633': /preacher curl pad.*chest height.*upper arm.*low cable handle.*underhand.*switch arms/i,
    '3563': /one handle.*high pulley.*overhand.*arm extended.*elbow close.*down toward the side/i,
    '1635': /lowest position.*preacher pad.*chest height.*upper arm.*underhand.*forearm.*switch arms/i,
    '0193': /handle.*waist height.*kneel facing.*one hand.*straight arm.*chest.*shoulder blade/i,
    '1723': /straight[- ]bar.*chest height.*face the machine.*step back.*upper arm stationary.*push the bar down.*elbow/i,
    '1636': /straight bar.*high pulley.*facing the machine.*underhand.*shoulder-width.*elbows.*forehead/i,
    '1637': /exercise ball.*underhand.*overhead.*elbows close to the ears.*upper arms still.*forehead/i,
    '0195': /cable preacher pad.*chest height.*upper arms.*underhand.*curl.*shoulders/i,
    '0196': /facing away.*rope between the legs.*both hands.*step forward.*hinge.*straight back.*hips forward.*glutes/i,
    '0197': /lat pulldown.*thighs secured.*pro lat bar.*overhand.*wider.*upper chest/i,
    '1638': /straight bar.*highest pulley.*underhand.*shoulder-width.*elbows close.*upper arms stationary.*thighs/i,
    '0201': /straight bar.*high pulley.*facing the machine.*overhand.*shoulder-width.*elbows close.*upper arms stationary.*full elbow extension/i,
    '0199': /straight bar.*high pulley.*overhand.*straight arms.*bar down.*thighs.*without bending the elbows/i,
    '0200': /rope.*high pulley.*overhand.*palms facing each other.*elbows close.*upper arms stationary.*separate the rope ends/i,
    '0202': /stirrup handle.*low pulley.*one foot back.*hinge forward.*straight back.*overhand.*chest.*shoulder blade/i,
    '0204': /low pulley.*facing away.*overhand.*arms straight in front.*elbows stationary.*pull.*triceps/i,
    '0205': /highest position.*sit facing.*feet flat.*overhand.*wider than the shoulders.*chest up.*upper chest/i,
    '0873': /low pulley.*face up.*both hands.*arms straight up.*knees.*upper body stable.*pelvis.*hips/i,
    '0206': /straight bar.*low pulley.*facing the machine.*underhand.*elbows close.*upper arms stationary.*shoulders/i,
    '2406': /straight bar.*highest pulley.*arm blaster.*facing the machine.*underhand.*shoulder-width.*upper arms fixed.*full elbow extension/i,
    '1413': /cable machine.*one handle.*palm down.*elbow close.*forearm.*shoulder.*switch arms/i,
    '0209': /cable preacher pad.*chest height.*upper arms braced.*reverse grip.*palms down.*elbows extended.*shoulders/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0190': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0191': /Primary muscles: Pectorals\.[\s\S]*Secondary muscles: Deltoids, Trapezius/i,
    '0192': /Primary muscles: Delts\.[\s\S]*Secondary muscles: Traps, Triceps/i,
    '1633': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '3563': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '1635': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0193': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '1723': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1636': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1637': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0195': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0196': /Primary muscles: Glutes\.[\s\S]*Secondary muscles: Hamstrings, Lower back/i,
    '0197': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '1638': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0201': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0199': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '0200': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0202': /Primary muscles: Delts\.[\s\S]*Secondary muscles: Trapezius, Rhomboids, Biceps/i,
    '0204': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders, Back/i,
    '0205': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Biceps, Rhomboids, Rear deltoids/i,
    '0873': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Hip flexors/i,
    '0206': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '2406': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1413': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0209': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})
...
test('reverse-grip cable batch prompts preserve movement and target details', () => {
  const technique = {
    '0210': /low pulley.*sit on a bench.*overhand.*palms down.*forearms.*thighs.*wrists.*curl.*lower/i,
    '0207': /straight bar.*high pulley.*facing the machine.*underhand.*shoulder-width.*palms up.*elbows close.*upper arms stationary.*full elbow extension/i,
    '0208': /sit facing.*feet flat.*straight back.*underhand.*shoulder-width.*pull.*torso.*retracting the shoulder blades/i,
    '1639': /rope.*low pulley.*preacher bench.*upper arms.*pad.*neutral grip.*palms facing each other.*shoulders/i,
    '1724': /rope.*high pulley.*facing away.*overhead.*palms down.*upper arms close.*ears.*perpendicular.*behind the head.*extend/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0210': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Wrists/i,
    '0207': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0208': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '1639': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1724': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('next cable batch prompts preserve movement and target details', () => {
  const technique = {
    '1725': /rope.*high pulley.*incline bench.*facing away.*overhead.*overhand.*elbows close.*upper arms stationary.*behind the head.*extend overhead/i,
    '1726': /rope.*lowest pulley.*lie face up.*head toward.*palms facing each other.*arms vertical.*upper arms stationary.*forehead.*extend/i,
    '1640': /rope handle.*low pulley.*preacher bench.*underhand.*palm up.*upper arm.*pad.*curl.*switch arms/i,
    '0211': /stability ball.*feet flat.*knees.*cable handle.*both hands.*lean back.*rotate the torso.*one hip.*other side/i,
    '2144': /seat.*cable handles.*sit upright.*feet flat.*shoulder height.*overhand grip.*press.*full arm extension.*return/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1725': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '1726': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1640': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0211': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques/i,
    '2144': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('seated cable batch prompts preserve movement and target details', () => {
  const technique = {
    '0212': /cable machine.*feet flat.*knees bent.*both hands.*behind the head.*abs.*curl.*chest.*knees/i,
    '1641': /cable machine.*feet flat.*back straight.*underhand.*palms[- ]up.*arms fully extended.*upper arms stationary.*shoulders.*lower fully/i,
    '0214': /bench facing.*cable machine.*feet flat.*knees slightly bent.*one handle.*arm extended.*retracting the shoulder blade.*elbow close.*alternate arms/i,
    '1642': /bench.*feet flat.*back straight.*cable handle.*elbow.*inside of the thigh.*upper arm stationary.*curl.*switch arms/i,
    '1643': /bench facing.*cable machine.*feet flat.*underhand.*palms[- ]up.*shoulder-width.*upper arms stationary.*elbows close.*shoulders.*lower fully/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0212': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Hip flexors/i,
    '1641': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0214': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '1642': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1643': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('cable raise and row batch prompts preserve movement and target details', () => {
  const technique = {
    '0215': /cable machine.*feet flat.*both handles.*overhand.*arms straight in front.*raise.*sides.*parallel.*lower/i,
    '0861': /cable row machine.*footrests.*knees slightly bent.*overhand.*straight back.*pull.*torso.*shoulder blades.*release/i,
    '0216': /cable machine.*feet flat.*one handle.*arm extended straight forward.*elbow slightly bent.*shoulder blades.*rotate.*body center.*switch arms/i,
    '2399': /cable machine.*feet flat.*knees slightly bent.*both hands.*arms straight in front.*brace the core.*rotate the torso.*across the body.*alternate/i,
    '0218': /cable row machine.*footrests.*knees slightly bent.*wide overhand.*palms-down.*back straight.*lower chest.*shoulder blades.*release/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0215': /Primary muscles: Rear deltoids\.[\s\S]*Secondary muscles: Upper back, Traps/i,
    '0861': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '0216': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: none/i,
    '2399': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques/i,
    '0218': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('standing cable shoulder and side-bend batch prompts preserve movement and target details', () => {
  const technique = {
    '0219': /facing away.*cable machine.*feet shoulder-width.*handles.*shoulder height.*overhand.*elbows bent.*press.*overhead.*arms.*straight.*lower/i,
    '0220': /facing the cable machine.*feet shoulder-width.*arms straight.*low cable handles.*elbows straight.*shrug.*ears.*lower/i,
    '0222': /beside the cable machine.*feet shoulder-width.*one handle.*near hand.*back straight.*core braced.*bend sideways.*knee.*switch sides/i,
    '0221': /one foot on a BOSU ball.*cable handle.*one hand.*other hand on the hip.*brace the core.*bend sideways.*balance.*switch sides/i,
    '0223': /sideways to a low pulley.*feet shoulder-width.*knees slightly bent.*outside hand.*other hand on the hip.*bend sideways.*outside elbow.*hip.*switch sides/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0219': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps/i,
    '0220': /Primary muscles: Traps\.[\s\S]*Secondary muscles: Shoulders, Forearms/i,
    '0222': /Primary muscles: Obliques\.[\s\S]*Secondary muscles: Abs/i,
    '0221': /Primary muscles: Obliques\.[\s\S]*Secondary muscles: Abs/i,
    '0223': /Primary muscles: Obliques\.[\s\S]*Secondary muscles: Abs/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('cable squat, curl, wrist, reverse-fly, and crunch batch preserves details', () => {
  const technique = {
    '1717': /cable machine.*rope.*waist height.*squat.*chest up.*back straight.*overhand grip.*row.*chest.*elbows close.*shoulder blades/i,
    '1644': /cable machine.*low handle.*feet shoulder-width.*underhand.*palms(?: facing)? up.*arms extended.*squat.*curl.*shoulders.*elbows close/i,
    '0224': /cable machine.*handle held behind the hips.*overhand.*palms-down.*arms straight.*wrists upward.*lower/i,
    '0225': /D-handles.*shoulder height.*centered between.*overhand grip.*arms out.*elbow bend.*forward and across.*shoulder blades/i,
    '0226': /rope.*high pulley.*facing away.*behind the head.*elbows bent.*feet shoulder-width.*knees slightly bent.*crunch.*knees.*elbows.*thighs/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1717': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '1644': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0224': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Biceps/i,
    '0225': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Upper back/i,
    '0226': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('standing cable crunch, fly, hip, curl, and lift batch preserves details', () => {
  const technique = {
    '0874': /rope.*chest height.*facing away.*feet shoulder-width.*behind the head.*elbows bent.*abs.*crunch.*elbows.*knees.*return/i,
    '0227': /handles.*chest height.*facing away.*feet shoulder-width.*overhand.*palms-forward.*step forward.*tension.*core braced.*elbows slightly bent.*arms together.*chest/i,
    '0228': /low pulley.*facing away.*ankle cuff.*torso upright.*core braced.*leg straight backward.*glute.*switch sides/i,
    '0229': /facing the cable machine.*feet shoulder-width.*underhand.*palms facing up.*elbows close.*upper arms stationary.*curl.*shoulders.*biceps.*lower/i,
    '0230': /facing the cable machine.*both hands.*handle at waist height.*arms straight.*back upright.*diagonally.*opposite shoulder.*rotating the torso.*switch sides/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0874': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques/i,
    '0227': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '0228': /Primary muscles: Glutes\.[\s\S]*Secondary muscles: Hamstrings, Quads/i,
    '0229': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0230': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Obliques, Lower back/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('standing cable triceps, pulldown, rear-delt, and row batch preserves details', () => {
  const technique = {
    '0231': /cable machine.*feet shoulder-width.*one handle.*palm down.*arm extended parallel.*elbow close.*upper arm still.*behind the head.*extend.*switch sides/i,
    '0232': /rope.*highest pulley.*facing the cable machine.*feet shoulder-width.*overhand.*palms-down.*back straight.*core braced.*thighs.*release/i,
    '0233': /facing the cable machine.*feet shoulder-width.*step back.*rope.*palms facing each other.*back straight.*core engaged.*upper chest.*elbows wide.*shoulder blades.*release/i,
    '1727': /facing away.*cable machine.*feet shoulder-width.*overhead.*underhand.*elbow close.*upper arm stationary.*behind the head.*extend.*switch arms/i,
    '0234': /facing the cable machine.*feet shoulder-width.*V-bar.*overhand.*palms-down.*back straight.*core engaged.*torso.*shoulder blades.*return/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0231': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0232': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms, Shoulders/i,
    '0233': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Upper back, Biceps/i,
    '1727': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0234': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('cable rotation, twist-row, pulldown, seated-row, and reverse-fly batch preserves details', () => {
  const technique = {
    '0235': /feet shoulder-width.*knees slightly bent.*handle.*arm extended in front.*elbow slightly bent.*shoulder blade.*rotate.*outward.*elbow.*return/i,
    '0236': /V-bar.*chest height.*facing the cable machine.*feet shoulder-width.*overhand.*palms-down.*step back.*back straight.*core engaged.*row.*torso.*twisting.*untwist.*alternating sides/i,
    '0237': /rope.*highest pulley.*facing the machine.*feet shoulder-width.*palms down.*arms extended forward.*elbow bend.*lats.*thighs.*arms straight.*return/i,
    '0239': /cable row machine.*footrests.*knees slightly bent.*overhand.*palms-down.*back straight.*hip hinge.*handles.*torso.*shoulder blades.*extend/i,
    '0240': /D-handles.*low pulley.*face down.*flat bench.*palms down.*arms straight in front.*arms out to the sides.*parallel.*lower/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0235': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps/i,
    '0236': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '0237': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '0239': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms, Shoulders/i,
    '0240': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Upper back/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('next cable pull, curl, kickback, upright-row, and wrist-curl batch preserves details', () => {
  const technique = {
    '0244': /low pulley.*facing the machine.*left hand.*arm fully extended.*feet shoulder-width.*knees slightly bent.*back straight.*core engaged.*rotating the torso.*right.*switch sides/i,
    '1645': /incline bench.*back against the pad.*feet flat.*both cable handles.*underhand.*palms up.*arms fully extended.*upper arms stationary.*curl.*shoulders.*lower/i,
    '1728': /feet shoulder-width.*knees slightly bent.*one cable handle.*each hand.*palms facing inward.*elbows.*90 degrees.*upper arms stationary.*forearms backward.*full extension/i,
    '0246': /feet shoulder-width.*knees slightly bent.*overhand grip.*back straight.*core braced.*straight up.*chin.*leading with the elbows.*lower/i,
    '0247': /straight bar.*low pulley.*facing the machine.*feet shoulder-width.*underhand.*palms(?: facing)?[- ]up.*forearms.*bench.*wrists hanging.*curl the wrists.*forearms still.*lower/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0244': /Primary muscles: Lats\.[\s\S]*Secondary muscles: Shoulders, Biceps/i,
    '1645': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1728': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0246': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Biceps/i,
    '0247': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Biceps, Brachialis/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('lying row, captain-chair raise, dip, and chest-tap batch preserves details', () => {
  const technique = {
    '0248': /cambered barbell.*floor.*face down.*bench.*chest.*overhand.*legs straight.*feet.*row.*chest.*shoulder blades.*lower/i,
    '2963': /captain.?s chair.*backrest.*forearms.*arm pads.*upper body stable.*back straight.*legs straight.*parallel.*lower/i,
    '1430': /dip bars.*comfortable height|dip-pull-up cage.*hands.*wider.*arms straight.*knees bent.*ankles crossed.*chest up.*shoulders down.*(?:below|pass) the elbows.*press/i,
    '2462': /straight parallel bar.*palms down.*arms fully extended.*knees.*cross the ankles.*shoulders (?:are )?below the elbows.*push.*straighten the arms/i,
    '3216': /high plank.*hands slightly wider.*straight line.*elbows close.*tap the chest.*push back up.*alternate/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0248': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Forearms/i,
    '2963': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Hip flexors/i,
    '1430': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '2462': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '3216': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('decline sit-up, diamond push-up, and dumbbell curl batch preserves details', () => {
  const technique = {
    '0282': /decline bench.*feet secured.*knees bent.*hands behind the head.*brace the abs.*curl the torso.*knees.*lower under control/i,
    '0283': /high plank.*hands close together.*diamond.*thumbs and index fingers.*body straight.*elbows close.*chest lowers.*push back up/i,
    '0285': /one dumbbell in each hand.*palms forward.*upper arms stationary.*curl one dumbbell.*other stays down.*alternate arms/i,
    '2403': /feet shoulder-width.*one dumbbell in each hand.*palms forward.*arm blaster.*upper arms stationary.*curl one dumbbell.*alternate sides/i,
    '1646': /preacher bench.*one dumbbell in each hand.*palms facing the torso.*upper arms stationary.*curl one weight.*alternate arms/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0282': /Primary muscles: Abs\.[\s\S]*Secondary muscles: Hip flexors, Lower back/i,
    '0283': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Chest, Shoulders/i,
    '0285': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '2403': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1646': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('preacher, seated, balance, and alternating press batch preserves details', () => {
  const technique = {
    '1647': /preacher bench.*dumbbell in each hand.*palms facing up.*upper arms on the pad.*curl the right dumbbell.*left arm stays extended.*alternate arms/i,
    '1648': /upright on a bench.*dumbbell in each hand.*palms facing the torso.*back straight.*elbows close.*curl one dumbbell.*other stays down.*alternate arms/i,
    '0286': /feet shoulder-width.*dumbbell in each hand.*shoulder height.*press one dumbbell overhead.*other stays at shoulder height.*alternate sides/i,
    '1649': /feet shoulder-width.*dumbbells at the sides.*one foot balanced on an exercise ball.*upper arms stationary.*curl one dumbbell.*alternate arms/i,
    '1650': /stability exercise ball.*feet flat.*back straight.*dumbbell in each hand.*palms forward.*curl one dumbbell.*other stays down.*alternate arms/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1647': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1648': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0286': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Core/i,
    '1649': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms, Shoulders/i,
    '1650': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('Arnold press, pullover, bench squat, and lunge-curl batch preserves details', () => {
  const technique = {
    '0287': /bench with back support.*dumbbell in each hand.*shoulder level.*palms facing the body.*press both dumbbells overhead.*rotating the wrists.*palms face forward/i,
    '0288': /lie flat on a bench.*one dumbbell with both hands.*above the chest.*arms straight.*lower the dumbbell behind the head.*raise it back/i,
    '0290': /bench with back support.*dumbbell in each hand.*beside the chest.*palms forward.*press both dumbbells overhead.*full extension/i,
    '0291': /facing away from a bench.*feet shoulder-width.*hold a dumbbell at the sides.*bend the hips and knees.*glutes.*touch the bench.*drive through the heels/i,
    '1651': /dumbbell in each hand.*palms forward.*forward lunge.*curling both dumbbells.*rotate the torso.*front leg.*bowling motion/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0287': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Upper chest/i,
    '0288': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Lats/i,
    '0290': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Chest/i,
    '0291': /Primary muscles: Glutes\.[\s\S]*Secondary muscles: Quads, Hamstrings, Calves/i,
    '1651': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms, Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('balance, arm-blaster, reverse-grip, and squat-curl batch preserves details', () => {
  const technique = {
    '1652': /stability exercise ball.*feet flat.*raise one leg.*dumbbell in each hand.*palms forward.*raised leg steady.*curling both dumbbells/i,
    '1653': /one leg.*other leg extended behind.*dumbbell in each hand.*palms forward.*upper arms stationary.*curl both dumbbells.*maintaining balance/i,
    '2401': /stand upright.*dumbbell in each hand.*palms forward.*arm blaster.*upper arms fixed.*curl both dumbbells/i,
    '1654': /dumbbell in each hand.*palms facing down.*arms fully extended.*upper arms stationary.*curl both dumbbells.*without turning the palms/i,
    '1655': /feet shoulder-width.*dumbbell in each hand.*palms forward.*controlled squat.*chest up.*curling both dumbbells.*return to standing/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1652': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms, Shoulders/i,
    '1653': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
...
test('dumbbell cross-body curl, Cuban press, deadlift, and decline press batch preserves details', () => {
  const technique = {
    '1657': /dumbbell in each hand.*palms facing the body.*elbows close.*curl each dumbbell diagonally across the body.*opposite shoulder.*alternating sides/i,
    '0299': /feet shoulder-width.*dumbbell in each hand.*shoulder height.*palms down.*press both dumbbells overhead.*palms face forward/i,
    '2136': /feet shoulder-width.*dumbbell in each hand.*front of the hips.*rotate the upper arms outward.*beside the shoulders.*press overhead/i,
    '0300': /feet shoulder-width.*toes forward.*dumbbell in each hand.*hinge at the hips.*lower the dumbbells.*straight back.*drive through the heels/i,
    '0301': /decline bench.*feet secured.*head lower than the hips.*dumbbell in each hand.*lower.*sides of the chest.*press back up/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1657': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0299': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Upper back/i,
    '2136': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Upper back/i,
    '0300': /Primary muscles: Glutes\.[\s\S]*Secondary muscles: Hamstrings, Lower back/i,
    '0301': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('decline dumbbell fly, presses, and shrug variants preserve details', () => {
  const technique = {
    '0302': /decline bench.*feet secured.*head lower than the hips.*palms facing each other.*wide arc.*bring them back together/i,
    '0303': /decline bench.*feet secured.*palms facing each other.*lower the dumbbells.*sides of the chest.*press.*full extension/i,
    '1617': /decline bench.*dumbbell in one hand.*extend that arm.*lower.*shoulder.*elbow close.*switch arms/i,
    '0305': /decline bench.*45-degree angle.*face down.*chest and stomach supported.*dumbbell in each hand.*shoulders toward the ears.*without bending/i,
    '0304': /decline bench.*45-degree angle.*face down.*palms facing each other.*shoulder blades together.*lower under control/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0302': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders/i,
    '0303': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '1617': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Chest, Shoulders/i,
    '0305': /Primary muscles: Traps\.[\s\S]*Secondary muscles: Shoulders, Rhomboids/i,
    '0304': /Primary muscles: Traps\.[\s\S]*Secondary muscles: Shoulders, Rhomboids/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('decline extension, twist fly, finger curl, lunge extension, and front raise preserve details', () => {
  const technique = {
    '0306': /decline bench.*feet secured.*dumbbell in each hand.*upper arms vertical.*bend only the elbows.*beside the head.*extend the elbows/i,
    '0307': /decline bench.*feet secured.*dumbbells above the chest.*wide arc.*rotating the wrists.*palms turn forward/i,
    '1437': /bench.*forearms supported on the thighs.*palms facing up.*roll toward the fingertips.*curl the fingers closed.*wrists still/i,
    '1732': /dumbbell overhead with both hands.*elbows bent behind the head.*step forward into a lunge.*extending the elbows/i,
    '0309': /dumbbell in each hand.*palms facing down.*raise both.*forward to shoulder height.*torso still.*lower.*thighs/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0306': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0307': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '1437': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Wrist flexors, Wrist extensors/i,
    '1732': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders, Core/i,
    '0309': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Biceps/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('full-can raise and dumbbell curl variants preserve details', () => {
  const technique = {
    '0311': /feet shoulder-width.*dumbbell in each hand.*palms facing the body.*back straight.*core engaged.*arms out to the sides.*parallel to the ground/i,
    '1659': /exercise ball.*feet flat.*back straight.*dumbbell in each hand.*palms facing the body.*upper arms stationary.*curl both dumbbells.*balanced/i,
    '0312': /dumbbell in each hand.*palms facing the torso.*rotate the palms forward.*upper arms stationary.*curl both weights.*shoulder level/i,
    '2402': /arm blaster.*dumbbell in each hand.*palms facing the torso.*upper arms fixed.*curl both weights.*shoulder level/i,
    '1664': /dumbbell in each hand.*palms facing forward.*upper arms stationary.*curl the dumbbells as high as possible.*upper chest/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0311': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Rotator cuff/i,
    '1659': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0312': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '2402': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1664': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('incline press, fly, and curl variants preserve details', () => {
  const technique = {
    '3545': /incline bench.*dumbbell in each hand.*shoulder height.*press one dumbbell overhead.*other stays at the shoulder.*alternate arms/i,
    '0316': /incline bench.*45-degree angle.*back against the pad.*feet flat.*palms forward.*lower.*sides of the chest.*press.*starting position/i,
    '0318': /incline bench.*45-degree angle.*dumbbell in each hand.*palms forward.*arms.*fully extended.*upper arms stationary.*curl both dumbbells/i,
    '0317': /incline bench.*dumbbell in each hand.*palms facing forward.*arms fully extended.*upper arms stationary.*curl both weights/i,
    '0319': /incline bench.*45-degree angle.*dumbbell in each hand.*palms facing each other.*lie back.*wide arc.*squeeze the chest/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '3545': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '0316': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '0318': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0317': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0319': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('incline hammer curls and presses preserve details', () => {
  const technique = {
    '0320': /incline bench.*dumbbell in each hand.*palms facing the torso.*back against the bench.*upper arms stationary.*curl both weights.*shoulder level/i,
    '0321': /incline bench.*45-degree angle.*dumbbell in each hand.*palms facing each other.*press.*slightly inward.*full extension/i,
    '1618': /exercise ball.*dumbbell in each hand.*palms facing each other.*head, neck, and upper back.*press.*full extension/i,
    '0322': /incline bench.*dumbbell in each hand.*palms facing each other.*upper arms.*elbows hanging down.*rotate the palms forward.*curl both weights/i,
    '1619': /incline bench.*dumbbell in one hand.*resting on the thigh.*shoulder height.*palm faces inward.*press.*switch arms/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0320': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0321': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '1618': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Chest, Shoulders/i,
    '0322': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
    '1619': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders, Chest/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('incline ball press, lateral raise, palm-in press, raise, and rear lateral raise preserve details', () => {
  const technique = {
    '1620': /exercise ball.*dumbbell in one hand.*head, neck, and upper back.*palm facing inward.*press.*ceiling.*switch arms/i,
    '0323': /incline bench.*dumbbell in one hand.*resting on the thigh.*upper arm.*inside of the thigh.*raise.*side.*parallel to the floor.*switch arms/i,
    '0324': /incline bench.*45-degree angle.*dumbbell in each hand.*palms facing each other.*press.*full extension/i,
    '0325': /incline bench.*dumbbell in each hand.*resting on the thighs.*shoulder height.*palms forward.*press.*overhead.*lower.*shoulder height/i,
    '0326': /incline bench.*45-degree angle.*chest against the backrest.*dumbbell in each hand.*palms facing each other.*raise.*sides.*parallel to the ground/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '1620': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders, Chest/i,
    '0323': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Triceps/i,
    '0324': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
    '0325': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Triceps/i,
    '0326': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Rhomboids/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('incline row, shoulder raise, shrug, triceps extension, and twisted fly preserve details', () => {
  const technique = {
    '0327': /incline bench.*45-degree angle.*chest against the incline backrest.*dumbbell in each hand.*arms fully extended.*pull both dumbbells.*chest.*shoulder blades.*lower/i,
    '0328': /incline bench.*45-degree angle.*back supported.*dumbbell in each hand.*palms facing down.*thighs.*shoulder height.*palms face forward.*press overhead/i,
    '0329': /incline bench.*45-degree angle.*back supported.*dumbbell in each hand.*palms facing the body.*arms hang straight down.*shoulders toward the ears.*arms straight/i,
    '0330': /incline bench.*dumbbell in each hand.*palms facing inward.*arms overhead.*elbows close.*lower the dumbbells behind the head.*upper arms stationary.*extend/i,
    '0331': /incline bench.*45-degree angle.*dumbbell in each hand.*palms facing each other.*wide arc.*slight elbow bend.*palms face forward.*bring them together overhead/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0327': /Primary muscles: Upper back\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '0328': /Primary muscles: Serratus\.[\s\S]*Secondary muscles: Shoulders, Traps/i,
    '0329': /Primary muscles: Traps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0330': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0331': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Shoulders, Triceps/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('iron cross, lateral-front raise, lying extension, elbow press, and femoral curl preserve details', () => {
  const technique = {
    '0332': /feet shoulder-width.*dumbbell in each hand.*sides.*raise both arms.*parallel to the ground.*T shape.*lower/i,
    '0335': /feet shoulder-width.*dumbbell in each hand.*palms facing the body.*back straight.*core engaged.*sides.*shoulder height.*forward.*shoulder height/i,
    '0337': /flat on a bench.*feet flat.*one dumbbell with both hands.*palms facing each other.*upper arms stationary.*arc behind the head.*forearms.*parallel to the floor.*triceps/i,
    '0338': /flat on a bench.*dumbbell in each hand.*palms facing each other.*lower.*shoulders.*bending only the elbows.*upper arms stationary.*press/i,
    '0339': /flat on your back.*dumbbell.*lower abdomen.*bend both knees.*lower legs.*glutes.*weight stable.*extend the knees/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0332': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Triceps, Chest/i,
    '0335': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Biceps/i,
    '0337': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
    '0338': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Chest, Shoulders/i,
    '0339': /Primary muscles: Hamstrings\.[\s\S]*Secondary muscles: Glutes/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('lying hammer press, rear-delt raise, one-arm presses, and pronated extension preserve details', () => {
  const technique = {
    '0340': /flat on a bench.*dumbbell in each hand.*palms facing each other.*lower.*sides of the chest.*elbows.*90 degrees.*press.*full extension/i,
    '0341': /face down.*flat bench.*dumbbell in one hand.*palm facing inward.*arm.*straight down.*raise.*back.*shoulder blade.*switch arms/i,
    '0342': /flat on a bench.*one dumbbell.*palm facing the feet.*arm.*above the chest.*lower.*chest.*elbow close.*press.*switch arms/i,
    '0343': /flat on a bench.*one dumbbell.*shoulder level.*palm facing forward.*press.*fully extended.*lower.*switch arms/i,
    '0344': /flat on a bench.*one dumbbell.*palm facing down.*above the shoulder.*upper arm stationary.*behind the head.*bending the elbow.*switch arms/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0340': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '0341': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Upper back/i,
    '0342': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '0343': /Primary muscles: Chest\.[\s\S]*Secondary muscles: Triceps, Shoulders/i,
    '0344': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Shoulders/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})

test('lying rear raise, extensions, pronation, supination, and curls preserve details', () => {
  const technique = {
    '0345': /face down.*flat bench.*dumbbell in one hand.*arm straight.*lift.*side.*switch arms/i,
    '0346': /flat on a bench.*underhand grip.*above the shoulder.*upper arm stationary.*behind the head.*bending the elbow.*switch arms/i,
    '0347': /face down.*flat bench.*dumbbell in each hand.*arms extended straight down.*palms upward.*upper arms stationary.*curl.*palms.*face down/i,
    '0349': /flat on a bench.*dumbbell in each hand.*palms facing up.*arms fully extended.*upper arms stationary.*curl both dumbbells.*shoulders/i,
    '0350': /flat on a bench.*dumbbell in each hand.*palms facing up.*arms fully extended.*upper arms stationary.*curl both weights.*shoulder level/i,
  }
  for (const [id, pattern] of Object.entries(technique)) assert.match(promptFor(id, 'technique'), pattern)
  const muscles = {
    '0345': /Primary muscles: Shoulders\.[\s\S]*Secondary muscles: Traps, Upper back/i,
    '0346': /Primary muscles: Triceps\.[\s\S]*Secondary muscles: Forearms/i,
    '0347': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '0349': /Primary muscles: Forearms\.[\s\S]*Secondary muscles: Biceps, Shoulders/i,
    '0350': /Primary muscles: Biceps\.[\s\S]*Secondary muscles: Forearms/i,
  }
  for (const [id, pattern] of Object.entries(muscles)) assert.match(promptFor(id, 'muscles'), pattern)
})
